...
SELECT id, room_type, number_of_reviews FROM final_airbnb WHERE number_of_reviews > 600 ORDER BY number_of_reviews DESC;
...
-- INVESTIGATE: Should Williamsburg be crowned the most popular neighbourhood? - No, because "Harlem" shows up the same number of times in the neigbourhood column, 
-- but has 1,118 more (TOTAL) reviews, indicating it has had 1,118 more bookings than "Williamsburg".

SELECT DISTINCT neighbourhood FROM final_airbnb WHERE neighbourhood Like "williamsburg";

SELECT neighbourhood, SUM(number_of_reviews) FROM final_airbnb WHERE neighbourhood Like "williamsburg";
-- Output: 1315
SELECT neighbourhood, SUM(number_of_reviews) FROM final_airbnb WHERE neighbourhood Like "harlem";
-- Output: 2433
...
SELECT id, COUNT(reviews_per_month) FROM final_airbnb WHERE minimum_nights < 7 ORDER BY reviews_per_month DESC LIMIT 1;
...
SELECT host_name, COUNT(host_name) AS total_number_of_listings FROM final_airbnb GROUP BY host_name ORDER BY total_number_of_listings DESC LIMIT 1;
